...
public class BpNode implements Serializable {
    ...
    public void insert(Tuple key, Tuple value,BpTree tree) {
        if (isLeaf) {
            if (!isLeafToSplit()) {
                // 叶节点无需分裂
                ...
                for (int i = 0; i < leftSize; i++) {
                    ...
                    entries.get(i).node = left;
                }
                for (int i = 0; i < rightSize; i++) {
                    right.entries.add(entries.get(leftSize + i));
                    entries.get(leftSize + i).node = right;
                }
                // 不是根节点
                if (!isRoot) {
                    // 调整父子节点关系
...
//                    System.out.println("index:" + index);
            ...
            if (key.compare(entries.get(0)) < 0) {
                ...
                for (int i = 0; i < (entries.size() - 1); i++) {
                    if (key.compare(entries.get(i)) >= 0 && key.compare(entries.get(i + 1)) < 0) {
                        children.get(i + 1).insert(key,value, tree);
                        break;
                    }
                }
            }
...
//                rootNode.insertInParent(keyToParent);
     ...
     * 插入到当前叶子节点中,不分裂
     */
    private void insertInLeaf(Tuple key,Tuple value) {
        if (!isLeaf) {
    ...
    private void insertImpl(Tuple key,Tuple value) {
        //遍历插入
        for (int i = 0; i < entries.size(); i++) {
            if (entries.get(i).compare(key) == 0) {
                ...
                entries.add(i, key);
                entries.get(i).data = value;
                entries.get(i).node = this;
                return;
            }
        }
        // 插入到末尾
        entries.add(key);
        entries.get(entries.size()-1).data = value;
        entries.get(entries.size()-1).node = this;
    }
